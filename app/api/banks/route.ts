...
export async function GET() {
  ...
  const [linkedResult, providersResult] = await Promise.all([
    pool.query<{ id: number; bank_name: string; bank_short: string; account_number_last4: string }>(
      "SELECT id, bank_name, bank_short, account_number_last4 FROM linked_banks WHERE agent_id = $1 ORDER BY created_at DESC",
      [auth.agent.id]
    ),
  ...
  return NextResponse.json({
    ...
    banks: linkedResult.rows.map((r) => ({ id: r.id, bankName: r.bank_name, accountLast4: r.account_number_last4 })),
